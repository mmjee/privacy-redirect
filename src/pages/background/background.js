...
function redirectWikipedia (url, initiator) {
  ...
  if (url.search.length > 0) {
    const search = url.search.substring(1) // get rid of '?'
    const argstrings = search.split('&')
    for (let i = 0; i < argstrings.length; i++) {
      const args = argstrings[i].split('=')
      GETArguments.push([args[0], args[1]])
    }
  }
  let link = `${wikipediaInstance}${url.pathname}`
  const urlSplit = url.host.split('.')
  if (urlSplit[0] != 'wikipedia' && urlSplit[0] != 'www') {
    if (urlSplit[0] == 'm') { GETArguments.push(['mobileaction', 'toggle_view_mobile']) } else GETArguments.push(['lang', urlSplit[0]])
...
browser.runtime.onInstalled.addListener((details) => {
  ...
  if (details.reason === 'update') {
    browser.storage.sync.get(
      ...
      (result) => {
        if (result.whitelist) {
          const whitelist = result.whitelist.map((e) =>
            e.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')
          )
          browser.storage.sync.set({
